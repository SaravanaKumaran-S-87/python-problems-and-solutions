...
The second line of each test case contains NN distinct space-separated integers A_1, A_2,...An denoting the festival days. Note that the A_i are not necessarily given in sorted order.
Output Format
...
Test Case 2: Days 6, 13, 20 and 27 are Saturdays, and days 7, 14, 21, 287,14,21,28 are Sundays. The festivals fall on day 1, day 6, and day 23. This gives us 10 holidays in total — days 1, 6, 7, 13, 14, 20, 21, 23, 27, 28.
...
def festival_days(n,arr):
    leave_arr=[6,7,13,14,20,21,27,28]
    holidays=8
    for i in range(0,n):
        if arr[i] in leave_arr:
            continue
        else:
            holidays+=1
    return holidays
print(festival_days(2,[5,7]))
print(festival_days(3,[23,1,6]))
print(festival_days(1,[13]))